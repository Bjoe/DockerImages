...
unset DISTCLEAN CLEAN DOCKER_COMPOSE_TARGET DOCKER_HOME_DIR DOCKER_BUILD_DIR EXECUTE_SCRIPT
...
if [ -p "$DOCKER_COMPOSE_TARGET" ]; then
  echo "No docker compose target set. Set docker build target via -t. See docker-compose.yml"
  exit 1;
fi
...
docker-compose -f $BASE_DIR/docker-compose.yml run $DOCKER_COMPOSE_TARGET /bin/bash /source/docker/$EXECUTE_SCRIPT ${USER_ID} ${GROUP_ID} ${DOCKER_COMPOSE_TARGET} 2>&1 | tee $DOCKER_BUILD_DIR/$DOCKER_COMPOSE_TARGET-$BUILD_TYPE-build.log
